...
async function testWithUndici() {
  console.log('🧪 Testing Production with undici fetch');
  ...
  try {
    ...
    // Test 4: Email URL format
    console.log('\n4. Testing email URL format...');
    const emailUrl = `${PRODUCTION_FRONTEND}/verify-email?token=test123&email=test@example.com`;
    console.log('Email URL:', emailUrl);
    
    const emailResponse = await fetch(emailUrl);
    console.log(`Email page status: ${emailResponse.status}`);

    console.log('\n📋 Summary:');
    console.log('✅ All tests completed with undici');
